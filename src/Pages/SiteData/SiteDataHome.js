...
const SiteDataHome = () => {
  ...
  const handleSearch = (site) => {
    //console.log(site)
    if (search !== "") {
      fetch(`https://bl-operation-server-8udwslvjt-mdatislam.vercel.app/searchSite?site=${site}`)
        .then((res) => res.json())
        .then((data) => {
          //console.log(data);
          setSelectSite(data);
  ...
  const onSubmit = (data) => {
    ...
    fetch(`https://bl-operation-server-8udwslvjt-mdatislam.vercel.app/siteInfo/${data.siteId}`, {
      method: "PUT",
      headers: {
